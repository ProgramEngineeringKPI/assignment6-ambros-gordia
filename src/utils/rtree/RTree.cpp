...
Node::Node(int LVL, Rect b, vector<Vertex> &origin, float mid, bool over, char mode) : bound(b), lvl(LVL), l(nullptr), r(nullptr)
{
    for (int i = 0; i < origin.size(); i++)
    {
        if (mode == 'x')
        {
            if ((over && origin[i].pt.x >= mid) || (!over && origin[i].pt.x <= mid))
                v.push_back(origin[i]);
        }
        else if (mode == 'y')
        {
            if ((over && origin[i].pt.y >= mid) || (!over && origin[i].pt.y <= mid))
                v.push_back(origin[i]);
        }
        else if (mode == 'z')
        {
            if ((over && origin[i].pt.z >= mid) || (!over && origin[i].pt.z <= mid))
                v.push_back(origin[i]);
        }
    }
}
...
char RTree::makeDecision(Node *curr)
{
    ...
    if (curr->bound.down.x == curr->bound.up.x)
        dec[0].first -= 100;
    if (curr->bound.down.y == curr->bound.up.y)
        dec[1].first -= 100;
    if (curr->bound.down.z == curr->bound.up.z)
        dec[2].first -= 100;
    sort(dec, dec + 3);
    return dec[2].second;
}
...
{
    if (curr->v.size() < MAX_CAPACITY)
        return;

    destructBy(curr);

    if (curr->l != nullptr)
        recursiveBuild(curr->l);
    if (curr->r != nullptr)
        recursiveBuild(curr->r);
}

Node *RTree::recursiveSearch(Node *pos, Ray *ray)
{
    if (pos->l == nullptr && pos->r == nullptr)
        return pos;
    if (pos->l == nullptr)
    {
        if (tracer.intersectsBox((pos->l)->bound.up, (pos->l)->bound.down, ray))
            return recursiveSearch(pos->l, ray);
        return pos;
    }
    if (pos->r == nullptr)
    {
        if (tracer.intersectsBox((pos->r)->bound.up, (pos->r)->bound.down, ray))
            return recursiveSearch(pos->r, ray);
        return pos;
    }
...
void RTree::build(vector<Vertex> &v)
{
    ...
    for (int i = 0; i < v.size(); i++)
    {
        ...
        vx = v[i];
        vx.facets.clear();
        for (int j = 0; j < v[i].facets.size(); j++)
            vx.facets.push_back(v[i].facets[j]);
        root->v.push_back(vx);
    }
    recursiveBuild(root);
}
...
pair<Vector, Vector> RTree::find(Ray *ray)
{
    ...
    if (!tracer.intersectsBox(root->bound.up, root->bound.down, ray))
        return ans;
